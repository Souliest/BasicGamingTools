...
// ---- Update all ----
function updateAll() {
    updateStats(); updateChips(); updateAvgStats();
    updateEntryLog(); updateTimeStats(); redrawCharts();
}
function redrawCharts() { drawGainChart(); drawTimeChart(); }
...
function updateAvgStats() {
    ...
    if (n === 0) {
        ['avgTotal','avgSmooth5','avgSmooth10'].forEach(id => document.getElementById(id).textContent = '—');
        return;
    }
...
function updateTimeStats() {
    if (gains.length === 0) {
        ['tMin','t15','tHour'].forEach(id => document.getElementById(id).textContent = '—');
        return;
    }
    const now = Date.now();
    const elapsed = (now - startTime) / 60000;
    const total   = gains.reduce((s,g) => s+g.xp, 0);
    function isReal(w) { return elapsed >= w; }
    ...
    function fmtRate(w, r) { return (isReal(w)?'':'~') + fmt(r*w); }
...
function drawGainChart() {
    ...
    function drawAligned(data, color, dash=[]) {
        ...
        ctx.setLineDash(dash); ctx.strokeStyle = color; ctx.lineWidth = 1.8; ctx.lineJoin = 'round';
        ctx.beginPath();
        data.forEach((v,i) => {
...
function drawTimeChart() {
    ...
    const now = Date.now();
    let cumulative = 0;
    const points = gains.map(g => { cumulative += g.xp; return { min:(g.ts-startTime)/60000, cum:cumulative }; });
    ...
    for (let i=0; i<=4; i++) {
        ...
        ctx.beginPath(); ctx.moveTo(pad.left,y); ctx.lineTo(w-pad.right,y); ctx.stroke();
        ctx.fillStyle = getCSSVar('--muted'); ctx.font = '9px Share Tech Mono, monospace'; ctx.textAlign = 'right';
    ...
    for (let i=0; i<=4; i++) {
        ...
        ctx.fillStyle = getCSSVar('--muted'); ctx.font = '9px Share Tech Mono, monospace';
    ...
    series.slice(1).forEach(pt => {
        ctx.beginPath();
        ctx.arc(pad.left+(pt.min/maxMin)*chartW, pad.top+chartH*(1-pt.cum/range), 3, 0, Math.PI*2);
        ctx.fill();
    });
}
